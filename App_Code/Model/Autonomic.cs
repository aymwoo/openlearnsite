using System;
namespace LearnSite.Model
{
	/// <summary>
	/// Autonomic:实体类(属性说明自动提取数据库字段的描述信息)
	/// </summary>
	[Serializable]
	public partial class Autonomic
	{
		public Autonomic()
		{}
		#region Model
		private int _aid;
		private int? _asid;
		private string _anum;
		private string _aname;
		private int? _ayid;
		private int? _afid;
		private string _atype;
		private string _afilename;
		private string _aurl;
		private int? _alength;
		private int? _ascore;
		private DateTime? _adate;
		private string _aip;
		private int? _avote;
		private int? _aegg;
		private bool _acheck;
		private string _aself;
		private bool _agood;
		private int? _ayear;
		private int? _agrade;
		private int? _aclass;
		private int? _aterm;
		private int? _ahit;
		private bool _aoffice;
		private bool _aflash;
		private bool _aerror;
		/// <summary>
		/// 
		/// </summary>
		public int Aid
		{
			set{ _aid=value;}
			get{return _aid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Asid
		{
			set{ _asid=value;}
			get{return _asid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string Anum
		{
			set{ _anum=value;}
			get{return _anum;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string Aname
		{
			set{ _aname=value;}
			get{return _aname;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Ayid
		{
			set{ _ayid=value;}
			get{return _ayid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Afid
		{
			set{ _afid=value;}
			get{return _afid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string Atype
		{
			set{ _atype=value;}
			get{return _atype;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string Afilename
		{
			set{ _afilename=value;}
			get{return _afilename;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string Aurl
		{
			set{ _aurl=value;}
			get{return _aurl;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Alength
		{
			set{ _alength=value;}
			get{return _alength;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Ascore
		{
			set{ _ascore=value;}
			get{return _ascore;}
		}
		/// <summary>
		/// 
		/// </summary>
		public DateTime? Adate
		{
			set{ _adate=value;}
			get{return _adate;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string Aip
		{
			set{ _aip=value;}
			get{return _aip;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Avote
		{
			set{ _avote=value;}
			get{return _avote;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Aegg
		{
			set{ _aegg=value;}
			get{return _aegg;}
		}
		/// <summary>
		/// 
		/// </summary>
		public bool Acheck
		{
			set{ _acheck=value;}
			get{return _acheck;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string Aself
		{
			set{ _aself=value;}
			get{return _aself;}
		}
		/// <summary>
		/// 
		/// </summary>
		public bool Agood
		{
			set{ _agood=value;}
			get{return _agood;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Ayear
		{
			set{ _ayear=value;}
			get{return _ayear;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Agrade
		{
			set{ _agrade=value;}
			get{return _agrade;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Aclass
		{
			set{ _aclass=value;}
			get{return _aclass;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Aterm
		{
			set{ _aterm=value;}
			get{return _aterm;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? Ahit
		{
			set{ _ahit=value;}
			get{return _ahit;}
		}
		/// <summary>
		/// 
		/// </summary>
		public bool Aoffice
		{
			set{ _aoffice=value;}
			get{return _aoffice;}
		}
		/// <summary>
		/// 
		/// </summary>
		public bool Aflash
		{
			set{ _aflash=value;}
			get{return _aflash;}
		}
		/// <summary>
		/// 
		/// </summary>
		public bool Aerror
		{
			set{ _aerror=value;}
			get{return _aerror;}
		}
		#endregion Model

	}
}

